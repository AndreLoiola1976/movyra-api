-- V3__mvp_constraints_and_barber_user_link.sql
-- Purpose:
-- 1) Avoid fragile uniqueness on customer phone (US phone formatting/normalization will come later)
-- 2) Link barbers to users to prevent identity mismatch between login (users) and barber records

-- 1) Drop UNIQUE(tenant_id, phone) from app.customers if it exists (name is auto-generated)
DO $$
DECLARE
...
-- 2) Link barbers to users (optional for existing data, but critical for consistency)
ALTER TABLE app.barbers
  ADD COLUMN IF NOT EXISTS user_id uuid;

-- Ensure one barber profile per user (within the whole platform)
...
BEGIN
  IF NOT EXISTS (
    ...
    JOIN pg_namespace n ON n.oid = t.relnamespace
    WHERE n.nspname = 'app'
      AND t.relname = 'barbers'
      AND c.conname = 'uk_barbers_user_id'
  ) THEN
    ALTER TABLE app.barbers
      ADD CONSTRAINT uk_barbers_user_id UNIQUE (user_id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE n.nspname = 'app'
      AND t.relname = 'barbers'
      AND c.conname = 'fk_barbers_user_id'
  ) THEN
    ALTER TABLE app.barbers
